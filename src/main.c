...
{
    char *cmd = NULL;
    char *args_before_pipe[BUFFERSIZE];
    char *args_after_pipe[BUFFERSIZE];
    ...
    while (getline(&cmd, &len, stdin) != -1 && strcmp(cmd, "exit\n") != 0)
    {
        ...
        strtok(cmd, "\n");
        parseCommand(cmd,args_before_pipe);
        ...
        if (pid1 == 0)
        {
            ...
            if (execvp(args_before_pipe[0], args_before_pipe) == -1)
            {
        ...
        if (pid2 == 0)
        {
            ...
            if (execvp(args_after_pipe[0], args_after_pipe) == -1)
            {
                perror(RED "Erreur de execlp"RESET);
                return EXIT_FAILURE;
            }
